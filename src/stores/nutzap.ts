  ...
  actions: {
    ...
    async generateP2PKKeys() {
      ...
      if (!walletStore.mnemonic) {
        throw new Error('Mnemonic not found in wallet store. Cannot derive P2PK keys.');
      }
      ...
      this.p2pkPublicKey = bytesToHex(schnorr.getPublicKey(p2pkHdKey.privateKey));
    ...
    async publishNutzapProfile() {
      ...
      if (!nostrStore.pubkey) {
        throw new Error('Nostr user not set. Please configure your Nostr key first.');
      }
      if (!this.p2pkPublicKey || this.configuredMints.length === 0 || this.configuredRelays.length === 0) {
        throw new Error('Profile is incomplete. Please generate a P2PK key and select mints and relays.');
      }

      const eventTemplate = {
        kind: 10019,
        content: 'My Nutzap profile for receiving private Bitcoin payments via Cashu.',
        tags: [
    ...
    startNutzapListener() {
      ...
      console.log('Started listening for incoming Nutzaps on relays:', this.configuredRelays);
    },
    ...
    async handleNutzapEvent(event: any) {
      ...
      try {
        ...
        if (!uTag || !this.configuredMints.includes(uTag[1])) {
          console.warn(`Ignoring nutzap from untrusted mint: ${uTag ? uTag[1] : 'N/A'}`);
          return;
        }
        ...
        // Redemption Step
        console.log(`Attempting to redeem ${proofs.reduce((a, p) => a + p.amount, 0)} sats...`);
